...
static func draw_on(canvas: CanvasItem, rot: RotSystem, perfil: RotProfile, dia: int) -> void:
	if not rot.active():
		return
	_rasto(canvas, rot)
...
static func _mancha(canvas: CanvasItem, rot: RotSystem) -> void:
	...
	canvas.draw_rect(Rect2(canto, Vector2(largura, alto)), WorldPalette.MANCHA)
...
static func _rasto(canvas: CanvasItem, rot: RotSystem) -> void:
	...
	canvas.draw_rect(caixa, WorldPalette.TRILHO)
