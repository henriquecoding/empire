# AUD-02 · A economia verdadeira: o que o CI afina é o que o jogo corre

```text
Porque    O modelo do §06 que o teste da asfixia afina dá 21→33 moedas/dia e o jogo produz 17 fixas; manutenção, ganância e custo da noite só existem no modelo; a produção não pede o trabalhador; a Colheita Forçada é sempre prejuízo.
Spec      docs/recovery/AUDITORIA-GAMEPLAY-2026-09-26.md
          docs/design/06-producao-conversao-e-comercio.md
          docs/design/49-economysystem-tres-circuitos-uma-passagem-por-fa.md
          docs/design/15-reis-ganancia-impulsos-e-sucessao.md
Depende   AUD-01
Contrato  o "Feito" é o contrato; os números vêm de data/ e da Spec
Feito     A produção medida no SimLoop bate com o modelo com tolerância escrita; um sorvedouro real na partida; um piloto que joga bem mede o §66 com a voz ligada; nenhum impulso nem modo do celeiro é dominado no teste
Fora      Comércio, diplomacia e dívida.
Estado    feito — a produção cresce, paga nobres e pede o trabalhador; a manutenção e o vagabundo da
          alvorada na partida; o D8 passa para o AUD-04 (o Cavador do dia 10, Q-101)
Horas     16
```
