...
- **Testes** — gdUnit4 6.2.1: 717 casos, 711 a passar, 6 saltados **com a razão escrita no próprio teste**,
...
| GPU e arte | Sem placa gráfica. A 26/09 tiraram-se capturas reais do jogo com `tools/captura.tscn` num ecrã virtual (Xvfb, Mesa em *software*): a cena, o HUD e o painel de contexto desenham-se, e a arte original aparece. Isso não aprova a arte nem a luz, e não substitui a GPU nem o Deck. |
...
| `data/i18n/strings.csv` | 339 chaves PT-PT e EN, zero por escrever. É o `strings.csv` único da §27. |
