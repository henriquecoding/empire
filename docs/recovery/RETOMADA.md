...
- **Testes** — gdUnit4 6.2.1: 332 casos, 324 a passar, 8 saltados **com a razão escrita no próprio teste**,
...
| O quê | Porquê |
|---|---|
| Jogabilidade | **Ninguém jogou** — com um teclado e olhos. O ciclo está todo lá e corre em *headless*: nove dos onze passos do `SimLoop` escritos (faltam o 9 e o 10, os dois da Fase 2), o mundo montado, a mancha a chegar ao crepúsculo e o núcleo a cair se não for defendido. O que **está** medido é a noite, e não a partida: o `tests/noite_do_07_test.gd` corre o cenário fechado do §07 e o `jogo_noite_test.gd` corre a noite no *greybox*. Os números do §07 não batem, e estão escritos na Q-073 em vez de calados — é o F1-16 que os vai afinar, e é a ele que falta um par de olhos. |
