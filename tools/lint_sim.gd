# tools/lint_sim.gd — portoes G1, G2, G4 e G6 sem abrir o gdUnit4 (§64, §65).
...
func _init() -> void:
	...
	problems.append_array(Rules.check_g6())
	for p in problems:
		printerr(p)
	print("lint_sim: %d problema(s) em G1, G2, G4 e G6" % problems.size())
